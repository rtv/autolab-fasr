...
CDestination::CDestination ( Stg::Model* mod )
{
  ...
  mLocation.x = mStgModel->GetPose().x;
  mLocation.y = mStgModel->GetPose().y;
...
//-----------------------------------------------------------------------------
tPoint2d CDestination::getLocation()
{
  return mLocation;
}
//-----------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
int CDestination::getWaitingPosition ( IRobotWaitInterface* robot, CPose &pose )
{
  bool fgFound = false;;
  int index = 0;
  IRobotWaitInterface* bot;
  ...
  pose.mX = mLocation.x + cos ( mQueueDirection ) * ( index ) * WAITING_SPACING;
  pose.mY = mLocation.y + sin ( mQueueDirection ) * ( index ) * WAITING_SPACING;
  pose.mYaw = NORMALIZE_ANGLE ( mQueueDirection - PI );

  return index;
}
//---------------------------------------------------------------------------
bool CDestination::isNear ( CPose pose )
{
  float dist;
...
//---------------------------------------------------------------------------
float CDestination::getDistance ( CPose pose )
{
  return EUCLIDIAN ( mLocation.x, mLocation.y, pose.mX, pose.mY );
}
//---------------------------------------------------------------------------
CPose CDestination::getPose()
{
  return CPose ( mLocation.x, mLocation.y, mQueueDirection );
}
//---------------------------------------------------------------------------
void CDestination::setLocation ( tPoint2d point )
{
  mLocation.x = point.x;
  mLocation.y = point.y;
}
