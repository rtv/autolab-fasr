...
//-----------------------------------------------------------------------------
CWaitProbPolicyRobot::CWaitProbPolicyRobot ( Stg::ModelPosition* modPos )
    : ABaseRobot ( modPos )
{
  mWaitDuration = 0;
...
//-----------------------------------------------------------------------------
CWaitProbPolicyRobot::~CWaitProbPolicyRobot()
{
}
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::getRobotVisData ( int id, char* buffer, int &len )
{
  ABaseRobot::getRobotVisData ( id, buffer, len );
}
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::selectWorkTaskPolicy()
{
  mWaitDuration = 0.0;
  mTaskStartedTimestamp = mSimTime;
}
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::selectNewTask()
{
  mTaskIndex = mTaskIndex + 1;
  if ( mTaskIndex >= mWorkTaskVector.size() ) {
    mTaskIndex = 0;
    mFgSendToDepot = true;
    if ( mBatteryLevel < 0.6 )
      mState = GOTO_CHARGER;
    else
...
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::startupPolicy()
{
  /*
    if ( mRobotId <= 2 )
      mTaskIndex = 0;
...
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::leaveChargerPolicy()
{
  if ( mFgSendToDepot )
    mState = GOTO_DEPOT;
  else
...
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::waitAtSourcePolicy()
{
  float r;
  mWaitDuration = mWaitDuration + mSimInterval;
...
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::waitAtChargerPolicy()
{
}
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::unallocatedPolicy()
{
...
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::pickupCompletedPolicy()
{
...
//-----------------------------------------------------------------------------
void CWaitProbPolicyRobot::waitingAtPickupPolicy()
{
  float waitTime;
